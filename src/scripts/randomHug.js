...
const apiURL = `${baseUrl}/api/v1/random/hug`;

function replaceSlashes(string) {
    return string.replace(/\//g, '%2F');
}

async function getHug(){
    const response = await fetch(apiURL);
    const data = await response.json();
    const hug = data.url;
    document.querySelector("#hug").src = hug;
    document.querySelector("#download").href = `${baseUrl}/api/v1/download?file=${replaceSlashes(encodeURI((hug)))}`;
}
